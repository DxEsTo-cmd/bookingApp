        ...
        public async Task CreateAsync(Booking model)
        {
            try
            {
                ...
                await dbContext.Database.ExecuteSqlCommandAsync(
                    $"EXEC @retVal = [Booking.Create] {model.ResourceId}, {model.Price}, '{model.StartTime.ToString("yyyy-MM-dd HH:mm:ss")}', '{model.EndTime.ToString("yyyy-MM-dd HH:mm:ss")}', '{model.CreatedUserId}', '{model.Note}'",
                    param);
