...
namespace BookingApp.Controllers
{
    ...
    public class FolderController : EntityControllerBase
    {
        ...
        [ProducesResponseType(404)]
        [ProducesResponseType(500)]
        [Route("api/folder/update-image/{id}")]
        [Authorize(Roles = RoleTypes.Admin)]
        public async Task<IActionResult> UpdateImage(int id, IFormFile filesData)
        {
            try
            {
                var folder =  await service.GetDetail(id);
                var imagePath = await service.SaveFolderImage(Request.Form.Files[0]);
                folder.Image = imagePath;
                await service.Update(id, UserId, folder);
            }
