...
declare @ResourceID int = 7;
declare	@Price float = 0;
declare	@StartTime DateTime = '2020-05-13 02:50:00';
declare	@EndTime DateTime = '2020-05-13 03:12:00';
declare	@UserID nvarchar(450) = 'e4be035a-cb0e-47be-b396-fbb0ea179cbd';
declare @Note nvarchar(MAX) = N'@retVal int output';
-- SET NOCOUNT ON added to prevent extra result sets from
	...
	DELETE Bookings
	WHERE ABS(DATEDIFF(minute,Bookings.EndTime, @StartTime)) = 22;
	...
	Begin
		...
		--Select Bookings.StartTime,Bookings.EndTime, Bookings.TerminationTime , Bookings.CreatedUserId
		--				From Bookings 
		--				Where Bookings.ResourceId = 7;



		Select Bookings.StartTime,Bookings.EndTime, Bookings.TerminationTime , Bookings.CreatedUserId
		From Bookings 
		Where Bookings.Id = 16251;

		--set @TimeDif = ABS(DATEDIFF(minute,@BookingEndTime, @RequestStartTime))
		--If @BookingPerUserTimeout <> 0 AND @RequestUserID = @BookingUserID And   @TimeDif > @BookingPerUserTimeout --17:15 >= 01:36		

		--SET @CountBooksInSameTime =(
		--	Select (SELECT [dbo].[Booking.IsRangeAvailable](
		--					@StartTime, 
		--					@EndTime, 
		--					@UserID,
		--					Bookings.StartTime, 
		...
		-- verify is no booking in same time
			...
			Select ABS(DATEDIFF(minute,Bookings.EndTime, @StartTime)), Bookings.StartTime,Bookings.EndTime, Bookings.TerminationTime , Bookings.CreatedUserId
						From Bookings 
						Where Bookings.ResourceId = @ResourceID 
						AND
		...
		Insert Into Bookings(ResourceID, Price, StartTime, EndTime, CreatedTime, UpdatedTime, CreatedUserID, UpdatedUserID, Note)
		Output INSERTED.Id Into @InsertResult
		Values(@ResourceID,@Price ,@StartTime, @EndTime, @BookingTimeStamp, @BookingTimeStamp, @UserID, @UserID, @Note);
