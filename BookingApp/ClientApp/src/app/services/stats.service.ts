...
export class StatsService {
  ...
  downloadFile(type: string, start: Date, end: Date, interval: string): Observable<HttpEvent<Blob>> {
    let startString = start.toDateString();
    let startDay = startString.substr(8, 2);
    let startMonth = startString.substr(4, 3);
    let startYear = startString.substr(11, 4);
    ...
    return this.http.request(new HttpRequest(
      'GET',
      `${this.url}/bookings-${type}/download?startTime=${startDay}%20${startMonth}%20${startYear}&endTime=${endDay}%20${endMonth}%20${endYear}&interval=${interval}`,
      null,
      {
        reportProgress: true,
        responseType: 'blob'
