...
import { MapCoordinates } from '../models/coordinate';
...
constructor(private http: HttpClient) {
  ...
  setUpCoordinates(coordinates: MapCoordinates): Observable<any> {
    return this.http.post(this.path + '/marker', coordinates);
  }
  ...
  deleteAllRelatedCoordinates(id: number): Observable<any> {
    return this.http.delete(this.path + '/coordinates/' + id);
  }
  ...
  updateImage(resource: Resource, formData: FormData): Observable<any> {
    const uploadReq = new HttpRequest('POST', this.path + '/update-image/' + resource.id, formData, {
      reportProgress: true,
    });
  ...
  updateAlreadyUploadedImage(resource: Resource, images: Array<string>): Observable<any> {
    const uploadReq = new HttpRequest('POST', this.path + '/update-image/already-upload/' + resource.id, images, {
      reportProgress: true,
    });
