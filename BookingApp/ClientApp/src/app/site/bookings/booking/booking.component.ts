...
export class BookingComponent implements OnInit {
  ...
  convertToDateTime(date: Date, time: Date) {
    ...
    let timeDate = this.parseTime(date);
  ...
  onSubmit() {
    if (this.mode == "edit") {
      if (this.form.valid) {
        ...
        let time = moment(this.startTime().value, ["h:mm A"]).format("HH:mm:ss");
        let data = moment(this.startDate().value).format("YYYY-MM-DD");
        let dateTime = new Date(data + 'T' + time );
        newValue.startTime = dateTime;
