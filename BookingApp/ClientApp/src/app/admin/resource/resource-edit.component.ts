...
import { MapCoordinates, Marker } from '../../models/coordinate';
...
export class ResourceEditComponent implements OnInit {
  ...
  initializeForm() {
    this.resourceForm = this.fb.group({
      ...
      price: [this.model.price, [Validators.required, Validators.min(1)]],
  ...
  onSubmit() {
    ...
    changes['price'] = this.model.price != formData.price;
    this.model.price = formData.price;
    ...
    if (this.updateMode) {
      this.resourceService.updateResource(this.model)
        .subscribe(result => {
          let alreadyUploadImage : Array<string> = [];
          this.imagesUpload.forEach(element => {
            if(element.file)
            {
              this.resourceService.updateImage(this.model,element.file).subscribe( result =>
              {
            ...
            {
              alreadyUploadImage.push(element.imageUrl);
            }
          });
          this.resourceService.updateAlreadyUploadedImage(this.model, alreadyUploadImage).subscribe(result =>
            {
              console.log('Image Updated');
            });

          this.resourceService.deleteAllRelatedCoordinates(this.model.id).subscribe(() => {
            let mapCoordinates: MapCoordinates = new MapCoordinates();

            this.markers.forEach(element => {
              mapCoordinates.coordinates.push(new Marker(element.lat, element.lng))
            });
            mapCoordinates.id = this.model.id;
    
            this.resourceService.setUpCoordinates(mapCoordinates).subscribe(() => {console.log('Coordinates loaded')});  
          });
